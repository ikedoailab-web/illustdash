...
export function ProjectForm({ onSuccess, onCancel, initialDeliveryDate, initialData = null, isModal = false }) {
    ...
    const [formData, setFormData] = useState(
        initialData || {
            title: '', category: 'キャラデザ', price: '', hours: '',
    ...
    const recentProjectNames = useMemo(() => {
        return [...new Set(projects.map(p => p.title))].slice(0, 5);
    }, [projects]);
    ...
    return (
        <div className={`space-y-6 ${!isModal ? "animate-in fade-in duration-500 slide-in-from-bottom-4 max-w-3xl mx-auto" : ""}`}>
            <div className="flex items-center justify-between">
                <div>
                    <h1 className={`${isModal ? "text-2xl" : "text-3xl"} font-bold tracking-tight`}>{isEditMode ? '案件の編集' : '新規案件登録'}</h1>
                    <p className="text-foreground/60 mt-1">{isEditMode ? '内容を修正して保存してください' : 'イラスト案件の詳細を入力して追加してください'}</p>
                </div>
                {isModal && onCancel && (<button onClick={onCancel} className="p-2 hover:bg-muted rounded-full transition-colors text-foreground/50 hover:text-foreground"><X className="h-5 w-5" /></button>)}
            </div>
                ...
                <form onSubmit={handleSubmit} className={`${isModal ? "py-4 space-y-6" : "p-6 md:p-8 space-y-8"}`}>
                    <div className="space-y-4">
                        ...
                        <div className="relative">
                            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><Type className="h-5 w-5 text-foreground/40" /></div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
                        <div className="space-y-4">
                            ...
                            <div className="relative group">
                                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><Tag className="h-5 w-5 text-foreground/40 group-focus-within:text-primary transition-colors" /></div>
                                <select name="category" value={formData.category} onChange={handleChange} className={inputClass + " appearance-none"}>
                                    <option value="キャラデザ">キャラデザ</option>
                                    <option value="立ち絵">立ち絵</option>
                                    <option value="背景">背景</option>
                                    <option value="ロゴ・アイコン">ロゴ・アイコン</option>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="relative">
                                <div className={`absolute left-3 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full shadow-sm ${formData.status === '制作中' ? 'bg-blue-500' : formData.status === 'チェック待ち' ? 'bg-orange-500' : 'bg-green-500'}`}></div>
                                <select name="status" value={formData.status} onChange={handleChange} className={inputClass + " appearance-none font-medium"}>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="relative group">
                                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><Calendar className="h-5 w-5 text-foreground/40 group-focus-within:text-primary transition-colors" /></div>
                                <input type="date" name="deliveryDate" required value={formData.deliveryDate} onChange={handleChange} className={inputClass} />
                            </div>
                        </div>
                        <div className="space-y-4">
                    ...
                    <div className="pt-6 border-t border-border flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
                        {isEditMode ? (
                            <button type="button" onClick={handleDelete} className="w-full sm:w-auto flex items-center justify-center text-red-500 hover:text-red-600 hover:bg-red-500/10 font-medium py-3 px-6 rounded-xl transition-all">
                                <Trash2 className="w-4 h-4 mr-2" />案件を削除
                            </button>
                        ) : <div></div>}
                        <div className="w-full sm:w-auto flex gap-3">
                            {isModal && (<button type="button" onClick={onCancel} className="w-full sm:w-auto px-6 py-3 border border-border rounded-xl hover:bg-muted font-medium transition-colors">キャンセル</button>)}
                            <button type="submit" className="w-full sm:w-auto bg-primary hover:bg-primary/90 text-primary-foreground font-semibold py-3 px-8 rounded-xl shadow-sm hover:shadow-md transition-all active:scale-95">
